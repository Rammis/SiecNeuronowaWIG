    ...
    class Network
    {
        ...
        public void learning(){
            ...
            for (int i = 0; i < ilosc_pierwsza; i++)
            {
                ...
                for (int j = 0; j < ilosc_druga; j++)
                    wagi_druga[j][i] = wagi_druga[j][i] + parametrN * delty_druga[j] * (1 - Math.Pow(Math.Tanh(druga[j].getWynik()), 2));
            }
            for (int i = 0; i < ilosc_danych; i++)
            {

                for (int j = 0; j < ilosc_pierwsza; j++)
                    wagi_pierwsza[j][i] = wagi_pierwsza[j][i] + parametrN * delty_pierwsza[j] * (1 - Math.Pow(Math.Tanh(pierwsza[j].getWynik()), 2));
            }

            

            for (int i = 0; i < wagi_wynikowy.Count; i++)
                wagi_wynikowy[i] = wagi_wynikowy[i] + parametrN * deltaOgolna * (1 - Math.Pow(Math.Tanh(wynikowy.getWynik()), 2));
